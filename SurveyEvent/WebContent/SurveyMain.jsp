...
<body>
	...
	try{
		...
		while (rs.next()) {
			...
			c++;
			%>
			<td class="surv" width='20%'><a href="surveyCont.jsp?id=<%=rs.getString("id") %>"><h3><%=rs.getString("title") %><br></h3></a><br>
		...
		sql = "select * from surveys where date(endDate)<date_format(now(),'%Y-%m-%d') and writer=? order by id desc";
		pstmt = conn.prepareStatement(sql);
		...
		while (rs.next()) {
			...
			if(!rs2.next()){
				out.println("<script>alert('아직 당발하지 않은 이벤트가 있습니다. 빨리 당발하고 설문결과를 확인하세요');</script>");
